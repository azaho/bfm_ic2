...
def log(message, priority=-1, indent=0):
    ...
    print(f"[{current_time} G {gpu_memory_reserved:.1f}G R {ram_usage:.1f}G] {' '*4*indent}{message}")
...
n_processes = 14 # XXX make adaptive
...
with Pool(processes=n_processes) as pool:
    datasets = pool.map(load_single_dataset, training_config['train_subject_trials'])
log("Done.")
...
train_dataloader = DataLoader(
    ...
    num_workers=4,  # XXX make a proper varable for this + in eval code
...
evaluation = FrozenModelEvaluation_SS_SM(
    ...
    regression_n_jobs=5,  # XXX make a proper varable for this + in eval code
)
...
if wandb: wandb.init(project=training_config['wandb_project'], name="all_subj5_nocorr_rfft", id="all_subj5_nocorr_rfft")
for epoch_i in range(training_config['n_epochs']):
    ...
    for batch_idx, (batch, (subject_identifier, trial_id)) in enumerate(train_dataloader):
        ...
        for optimizer in optimizers:
            optimizer.step()
        log(f"Epoch {epoch_i+1}/{training_config['n_epochs']}, Batch {batch_idx+1}/{len(train_dataloader)} ({subject_identifier}_{trial_id}), Loss: {loss.item():.4f}")
    ...
    with torch.no_grad():
        ...
        log(f"Estimated time remaining: {hours:02d}:{minutes:02d}:{seconds:02d}")
